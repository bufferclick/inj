...
function loadDashboard(serverId) {
    ...
    fbGet('dashboards/' + serverId).then(function(data) {
        if (!data) {
            ...
            return fbPut('dashboards/' + serverId, data).then(function() {
                return data;
            });
        }
        return data;
...
function saveCurrentFile() {
    ...
    // Save to Firebase
    var encodedName = fKey(openFile);
    fbPut('dashboards/' + currentServerId + '/files/' + encodedName, fileObj).then(function(result) {
        console.log('File saved:', openFile, result);
...
function createNewFile() {
    ...
    var encodedName = fKey(name);
    fbPut('dashboards/' + currentServerId + '/files/' + encodedName, fileObj).then(function() {
...
function confirmDeleteFile() {
    ...
    var encodedName = fKey(n);
    fbDel('dashboards/' + currentServerId + '/files/' + encodedName).then(function() {
...
function runBot() {
    ...
    if (openFile) {
        ...
        var encodedName = fKey(openFile);
        fbPut('dashboards/' + currentServerId + '/files/' + encodedName, fo);
    }
...
function switchAdminTab(tab) {
    ...
    if (tab === 'create') {
        ...
        // Set today's date as default minimum
        var today = new Date().toISOString().split('T')[0];
        document.getElementById('premiumExpiry').setAttribute('min', today);
    }
...
function loadOverview() {
    Promise.all([fbGet('premiumCodes'), fbGet('premium'), fbGet('dashboards'), fbGet('siteAdmins')]).then(function(res) {
        ...
        container.innerHTML = sorted.map(function(e) {
            var sid = e[0];
            var d = e[1];
            var fc = (d.files && typeof d.files === 'object') ? Object.keys(d.files).length : 0;
...
function createPremiumCode() {
    ...
    if (!code || code === '-----') {
        errEl.textContent = 'Click the regenerate button to generate a code first.';
        errEl.style.display = 'block';
        return;
    }
...
function renderAllDashCards(dashboards) {
    ...
    if (entries.length === 0) {
        container.innerHTML = '<div class="empty-state"><h4>No dashboards found</h4><p>Dashboards will appear here when users create them.</p></div>';
        return;
    }
...
function adminStopBot(serverId) {
    fbPatch('dashboards/' + serverId, { running: false }).then(function() {
        ...
        setTimeout(function() { loadAllDashboards(); }, 300);
    });
}
...
function adminDeleteDash(serverId) {
    ...
    fbDel('dashboards/' + serverId).then(function() {
        toast('Dashboard deleted', 'info');
        setTimeout(function() { loadAllDashboards(); loadOverview(); }, 300);
    });
}
...
function loadSiteAdmins() {
    fbGet('siteAdmins').then(function(admins) {
        ...
        // Filter and display valid admins only
        for (var uid in admins) {
            if (admins[uid] && typeof admins[uid] === 'object') {
                html += '<div class="admin-item">' +
                    '<div class="admin-item-info">' +
                        '<div>' +
                            '<div class="admin-item-label">' + esc(admins[uid].username || 'Admin') + '</div>' +
...
// Editor line numbers & dirty tracking
